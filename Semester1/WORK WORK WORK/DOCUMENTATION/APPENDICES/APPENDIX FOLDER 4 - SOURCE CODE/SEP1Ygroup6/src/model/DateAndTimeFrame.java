...
public class DateAndTimeFrame
{
  ...
  /**
   * A overloaded constructor to initialise instance variables
   *
   * @param date            from which the program receives the date in a MyDate format
   * @param startTime       from which the program receives the startTime in a MyTime format
  ...
  public boolean checkForOverlaps(DateAndTimeFrame other) //24 time units
  {
    ...
    else if (this.startTime.equals(other.startTime)) // 6 time units
    {
    ...
    else if (this.startTime.isBefore(other.endTime) && other.startTime.isBefore(this.endTime)) //5 time units
    {
      return true; //1 time unit
    }
  ...
  /**
   * A equals method which compares two objects of this class
   * @param o the object that is being comapred with "this"
  ...
  /**
   * A method which verifies if "this" object time is before the object received through the parameter
