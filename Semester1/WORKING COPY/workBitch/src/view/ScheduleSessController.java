...
public class ScheduleSessController {
    ...
    public void init(ViewHandler viewHandler, VIAModel model, Region root) {
        ...
        // change courses depending on semester
    ...
    public void displayAvailableClassrooms()
    {
        ...
        {
            if ( (LocalDate)dateSelect.getValue() != null)
            {
                ...
                availableClassrooms = model.returnAvailableClassrooms(chosenDate,new MyTime(hour,minutes),chosenNumLessons);

                ObservableList<Classroom> availableClassroomsShow =
                    FXCollections.observableArrayList(availableClassrooms);
    ...
    private void scheduleButton()
    {
        ...
        String chosenCourse = courseChoice.getValue().toString();
        String chosenStartTime = startChoice.getValue().toString();
        Classroom chosenClassroom = (Classroom) (classroomChoice.getValue());
        int repeatTimes = (Integer) repeatChoice.getSelectionModel().getSelectedItem();
        int chosenNumLessons = (Integer) numberChoice.getSelectionModel().getSelectedItem();
        String[] parts = chosenStartTime.split(":");
        int hour = Integer.parseInt(parts[0]);
        int minutes = Integer.parseInt(parts[1]);
        LocalDate chosenDateLocal = (LocalDate) dateSelect.getValue();
        MyDate chosenDate = new MyDate(chosenDateLocal.getDayOfMonth(),
            chosenDateLocal.getMonthValue(), chosenDateLocal.getYear());

        Session toBeScheduled = new Session(chosenCourse, chosenNumLessons,
            new MyTime(hour, minutes), chosenDate, chosenClassroom);

        try
        {
            model.scheduleSession(toBeScheduled, chosenSemester, chosenClass);
            System.out.println(
                "Scheduled: " + chosenCourse + " for class " + chosenSemester + "" + chosenClass + "on the " + chosenDate + ": "
                    + chosenStartTime + " - " + toBeScheduled.getEndTime() + "in classroom: " + toBeScheduled.getClassroom());
            for (int i = 1; i <= repeatTimes; i++)
            {
                ...
                System.out.println("Scheduled: " + chosenCourse + " for class "
                    + chosenSemester + "" + chosenClass + "on the " + session.getSessionDate() + ": " + chosenStartTime + " - "
